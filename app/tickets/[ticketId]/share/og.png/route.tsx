...
export async function GET(request: Request, { params }: { params: { ticketId: string } }) {
  try {
    ...
    if (!ticket?.event_id) return createShareFallbackResponse("og", "ticket", "Entrada");
    ...
    if (!event || (status && !["active", "available"].includes(status))) {
      return createShareFallbackResponse("og", "ticket", "Entrada");
    }
    ...
    console.error("[share/og:ticket] render error", {
