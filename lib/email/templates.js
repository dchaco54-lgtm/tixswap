...
export function templateEventNewTicketAlert({
  recipientName,
  eventName,
  price,
  link,
  ...
  if (place) lines.push(`<p><b>Ubicación:</b> ${escapeHtml(place)}</p>`);
  ...
  const seatInfo = buildSeatInfo({ sector, sectionLabel, rowLabel, seatLabel });
  if (seatInfo) lines.push(`<p><b>Ubicación entrada:</b> ${seatInfo}</p>`);

  const safeLink = link ? escapeHtml(link) : null;
  const subjectEvent = String(eventName || "").trim();

  return {
    subject: subjectEvent
      ? `TixSwap - Nueva entrada publicada para ${subjectEvent}`
      : "TixSwap - Nueva entrada publicada",
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.5">
        <h2 style="margin:0 0 12px">Nueva entrada publicada</h2>
        <p>${greeting(recipientName)}</p>
        ${lines.join("")}
        ${safeLink ? `<p><a href="${safeLink}">Ver evento</a></p>` : ""}
        <p style="color:#666;font-size:12px;margin-top:16px">Este correo es automatico.</p>
      </div>
    `,
  };
...
export function templateEventAlertSubscribed({
  recipientName,
  eventName,
  link,
  venue,
  ...
  const safeLink = link ? escapeHtml(link) : null;
  const safeEvent = eventName ? escapeHtml(eventName) : "este evento";
  const place = [venue, city].filter(Boolean).join(", ");

  return {
    subject: eventName
      ? `TixSwap - Activaste alertas para ${eventName}`
      : "TixSwap - Activaste alertas de evento",
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.5">
        <h2 style="margin:0 0 12px">Alerta activada</h2>
        <p>${greeting(recipientName)}</p>
        <p>Te avisaremos cuando haya nuevas entradas para <b>${safeEvent}</b>.</p>
        ${place ? `<p><b>Ubicación:</b> ${escapeHtml(place)}</p>` : ""}
        ${safeLink ? `<p><a href="${safeLink}">Ver evento</a></p>` : ""}
        <p style="color:#666;font-size:12px;margin-top:16px">Este correo es automatico.</p>
      </div>
    `,
...
export function templateEventUpdated({
  recipientName,
  eventName,
  changeType,
  changeTypeDetail,
  ...
  const safeEvent = eventName ? escapeHtml(eventName) : null;
  const safeLink = link ? escapeHtml(link) : null;
  const safeType = changeType ? escapeHtml(changeType) : null;
  const safeTypeDetail = changeTypeDetail ? escapeHtml(changeTypeDetail) : null;
  const safeMessage = messageToUsers ? escapeHtml(messageToUsers).replace(/\n/g, "<br/>") : null;
  const safeWhatsapp = supportWhatsapp ? escapeHtml(supportWhatsapp) : null;

  const changesHtml = (changes || [])
    .map((c) => {
      const label = c?.label ? escapeHtml(c.label) : "Cambio";
      const before = c?.before != null ? escapeHtml(String(c.before)) : "—";
      const after = c?.after != null ? escapeHtml(String(c.after)) : "—";
      return `<li><b>${label}:</b> ${before} → ${after}</li>`;
    })
    .join("");

  return {
    subject: safeEvent
      ? `Evento actualizado: ${safeEvent}`
      : "Evento actualizado",
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.5">
        <h2 style="margin:0 0 12px">Evento actualizado</h2>
        <p>${greeting(recipientName)}</p>
        ${safeEvent ? `<p><b>Evento:</b> ${safeEvent}</p>` : ""}
        ${safeType ? `<p><b>Tipo de cambio:</b> ${safeType}${safeTypeDetail ? ` · ${safeTypeDetail}` : ""}</p>` : ""}
        ${changesHtml ? `<ul>${changesHtml}</ul>` : ""}
        ${safeMessage ? `<p><b>Mensaje:</b><br/>${safeMessage}</p>` : ""}
        ${safeLink ? `<p><a href="${safeLink}">Ver evento</a></p>` : ""}
        ${safeWhatsapp ? `<p>Soporte WhatsApp: <a href="${safeWhatsapp}">${safeWhatsapp}</a></p>` : ""}
        <p style="color:#666;font-size:12px;margin-top:16px">Este correo es automatico.</p>
      </div>
    `,
  };
