...
function getSupabaseAuth() {
  ...
  const tokenCookie = cookieStore.get(`sb-${projectRef}-auth-token`)?.value;
...
export async function updateProfile(updates) {
  try {
    ...
    if (typeof updates.phone === 'string') {
      allowed.phone = updates.phone.trim();
    }
    if (typeof updates.avatar_url === 'string') {
      allowed.avatar_url = updates.avatar_url.trim();
    }

    const keys = Object.keys(allowed);
    if (keys.length === 0) {
      throw new Error('No hay cambios permitidos');
    }
...
export async function findOpenChangeTicket(field) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('status', 'open')  // Estado normalizado
      .single();
...
export async function createProfileChangeTicket(field, requestedValue, reason = '') {
  try {
    ...
    const ticketData = {
      ...
      status: 'open',  // Estado normalizado (antes era 'abierto')
      user_id: user.id  // Agregar user_id para RLS
    };
