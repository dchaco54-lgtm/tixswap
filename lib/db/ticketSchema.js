...
const EVENT_FIELDS = [
  ...
  "city",
  "image_url",
  "nomination_enabled_at",
  "renomination_cutoff_hours",
  "renomination_max_changes",
];

let _cachedTicketColumns = null;
let _cachedEventColumns = null;

async function detectByInformationSchema(supabase, tableName) {
...
async function detectBySampleRow(supabase, tableName) {
...
export async function detectTicketColumns(supabase) {
  if (_cachedTicketColumns) return _cachedTicketColumns;

  if (!supabase || typeof supabase.from !== "function") {
    _cachedTicketColumns = new Set(["id"]);
    return _cachedTicketColumns;
  }

  // 1) Intento por information_schema (si está expuesto)
  try {
    const cols = await detectByInformationSchema(supabase, "tickets");
    if (cols) {
      _cachedTicketColumns = cols;
      return _cachedTicketColumns;
    }
  } catch {
    // noop
  ...
  try {
    const cols = await detectBySampleRow(supabase, "tickets");
    _cachedTicketColumns = cols || new Set(["id"]);
    return _cachedTicketColumns;
  } catch {
    _cachedTicketColumns = new Set(["id"]);
    return _cachedTicketColumns;
...
/**
 * Detecta columnas reales en `events` y las cachea (best-effort).
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @returns {Promise<Set<string>>}
 */
export async function detectEventColumns(supabase) {
  if (_cachedEventColumns) return _cachedEventColumns;

  if (!supabase || typeof supabase.from !== "function") {
    _cachedEventColumns = new Set(["id"]);
    return _cachedEventColumns;
  }

  try {
    const cols = await detectByInformationSchema(supabase, "events");
    if (cols) {
      _cachedEventColumns = cols;
      return _cachedEventColumns;
    }
  } catch {
    // noop
  ...
  try {
    const cols = await detectBySampleRow(supabase, "events");
    _cachedEventColumns = cols || new Set(["id"]);
    return _cachedEventColumns;
  } catch {
    _cachedEventColumns = new Set(["id"]);
    return _cachedEventColumns;
...
function buildEventSelect(eventColumns) {
  ...
  if (cols.size === 0) return "id";

  const parts = [];
  for (const key of EVENT_FIELDS) {
    if (cols.has(key)) parts.push(key);
  }
 ...
 * @returns {string}
 */
export function buildTicketSelect(columns, eventColumns) {
  ...
  if (cols.has("seller_id")) parts.push("seller_id");
  if (cols.has("seller_name")) parts.push("seller_name");
  ...
  if (cols.has("event_id")) {
    const eventSelect = buildEventSelect(eventColumns);
    parts.push(`event:events(${eventSelect})`);
  }
