...
/**
 * Verificar si estado es cerrado (final, no reabrible)
 */
...
/**
 * Verificar si se puede reabrir (solo resolved, 1 vez)
 */
export function canReopenTicket(status, reopenCount = 0) {
  const count = Number(reopenCount || 0);
  return status === TICKET_STATUS.RESOLVED && count < 1;
}
