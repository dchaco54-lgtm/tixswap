...
export async function finalizeTicketUpload({
  ...
  if (!hasTicketUploadsEventId) {
    console.warn(
      "[ticketUploads] ticket_uploads.event_id missing, using legacy-compatible flow"
    );
  }
  ...
  if (hasTicketUploadsEventId) {
    updatePayload.event_id = eventId || upload?.event_id || null;
  }
  ...
  if (hasTicketUploadsEventId) {
    selectFields.splice(3, 0, "event_id");
  }
